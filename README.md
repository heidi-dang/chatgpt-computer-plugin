...
- ChatGPT owns ordinary prompt execution. CPTR's task and monitor delegation tools reject omitted delegation opt-in, omitted model names, and bare connection/model identifiers that could trigger provider model discovery. An explicit delegation must use `delegate_to_cptr_model=true` plus `provider/model` or `agent:profile/model` after the user asks for that model.
