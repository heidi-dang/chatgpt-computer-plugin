...
- ChatGPT owns ordinary prompt execution. CPTR's task and monitor delegation tools reject delegated task/goal text without the literal `allow:delegation` marker, omitted model names, and bare connection/model identifiers that could trigger provider model discovery. An explicit delegation must include `allow:delegation` plus `provider/model` or `agent:profile/model` after the user asks for that model.
