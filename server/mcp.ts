...
export function createMcpServer(
  ...
  const publishBrowserSurface = (input: Record<string, unknown>, result: Record<string, unknown>) => {
    const commandResult = recordFrom(result.result);
    const commandPayload = recordFrom(commandResult.payload);
    ...
    const semanticOwner = result.owner
      ?? lease.owner
      ?? ((action === "command" || action === "approve_evaluate") ? "agent" : input.expected_owner);
    const semanticState = result.state
      ?? lease.state
      ?? (semanticOwner === "agent"
        ? "AGENT_CONTROL"
        : semanticOwner === "human"
    ...
    // Once a browser session has been rendered into one prompt, all later
    // command/handoff events must return to that same card. Falling back to the
    // process-wide active prompt can strand an older widget in AGENT_CONTROL
    // when another ChatGPT prompt becomes active between browser calls.
    const promptTicket = promptSessions.ticketForBrowserSession(sessionId) ?? currentPromptTicket();
    promptSessions.append(promptTicket, { type: "browser.surface", payload });
  };
  ...
  server.registerTool(
    "cptr_user_chrome",
    {
      ...
      description:
        "Control a user-approved Chrome extension connection through CPTR without changing the isolated cptr_chrome_browser tool. Use approve_pairing with the exact opaque pairing ID shown by the extension before the user claims it. Authenticated approval is bound to that high-entropy pairing ID; the extension-held claim secret remains required to claim the device credential. Start with list_devices, use list_tabs with device_id to inspect sanitized live tabs, or call open_session with only device_id to create and attach a dedicated Chrome window automatically; an explicit tab_id may still be supplied to control a specific approved tab. Use command for browser actions; batch groups up to 24 bounded allowlisted UI-input steps into one command to reduce round trips. transfer_lease performs explicit agent/human ownership handoff. Mutating browser actions require expected_epoch and are rejected before dispatch if the epoch is absent or stale. When browser work is complete and the agent still owns the lease, MUST transfer_lease from agent to none before the final response unless the user explicitly asked to keep browser control active; this ends the scoped Chrome debugger attachment and removes Chrome's debugging banner. Pairing inputs are redacted from telemetry, and the extension never receives the MCP bearer token.",
