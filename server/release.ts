...
export function currentPluginUpdateManifest(env: NodeJS.ProcessEnv = process.env): PluginUpdateManifest {
  return {
    ...
    summary: `CPTR Computer v${CPTR_APP_VERSION} reduces MCP control-path latency while keeping durable execution and memory safety semantics intact.`,
    changes: [
      ...
      "Keeps the existing MCP tool surface and security model unchanged; the schema revision advances because cptr_execute_task wait bounds changed.",
