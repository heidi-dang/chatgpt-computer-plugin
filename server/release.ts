...
export function currentPluginUpdateManifest(
  ...
  return {
    ...
    changes: [
      ...
      "Raises Traffic, Activity, and Diagnostics plugin flush defaults from 250 ms to 1000 ms to reduce backend write pressure while preserving bounded queues and shutdown flush semantics.",
      "Keeps the compatible MCP action names and security model intact while profiling and bounding the surface; the schema revision advances for the new wait bounds and diagnostics fields.",
      ...
      "Streams Direct Coding Worker command and fixed-test stdout/stderr through the same authenticated Workbench live terminal, with worker identity bound into the opaque command ticket so backend snapshot/SSE recovery resolves the correct isolated worktree.",
      "Keeps Direct Coding Worker lifecycle metadata compact while preserving the unified Live Terminal command target instead of detaching it when worker status updates arrive.",
