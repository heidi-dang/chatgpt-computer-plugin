...
const diagnosticsModule = await import("../server/mcp-diagnostics.js").catch(() => ({} as Record<string, unknown>));
...
test("MCP diagnostics emitter is bounded, allowlist-only, and sanitizes summaries", async () => {
  ...
  for (const forbidden of ["authorization", "headers", "arguments", "result", "top-secret", "/home/"]) {
    assert.equal(encoded.includes(forbidden), false, `diagnostics must not contain ${forbidden}`);
  }
...
test("ComputerClient diagnostics delivery uses the dedicated endpoint with generic failures", async () => {
  ...
  assert.equal(typeof (computer as unknown as { ingestMcpDiagnostics?: unknown }).ingestMcpDiagnostics, "function");
  await (computer as unknown as { ingestMcpDiagnostics: (events: Array<Record<string, unknown>>) => Promise<void> }).ingestMcpDiagnostics([
    {
      kind: "latency",
      version: 1,
  ...
  await assert.rejects(
    (failing as unknown as { ingestMcpDiagnostics: (events: Array<Record<string, unknown>>) => Promise<void> }).ingestMcpDiagnostics([]),
    (error: unknown) => {
...
test("ComputerClient synchronously observes real backend request duration and outcome", async () => {
  ...
  const computer = new ComputerClient({
    baseUrl: "http://cptr.test",
    token: "test-token",
    fetchImpl: async () => new Response(JSON.stringify({ workspaces: [] }), { status: 200 }),
  });
  assert.equal(typeof (computer as unknown as { setRequestObserver?: unknown }).setRequestObserver, "function");
  (computer as unknown as { setRequestObserver: (observer: (value: Record<string, unknown>) => void) => void }).setRequestObserver(
...
test("ComputerClient observer receives sanitized backend failures but cannot alter request results", async () => {
  ...
  const computer = new ComputerClient({
    baseUrl: "http://cptr.test",
    token: "test-token",
    fetchImpl: async () => new Response(JSON.stringify({ detail: "backend unavailable" }), { status: 503 }),
  });
  (computer as unknown as { setRequestObserver: (observer: (value: Record<string, unknown>) => void) => void }).setRequestObserver(
    (observation) => {
