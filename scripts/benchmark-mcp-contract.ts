...
async function measure(toolSurface: "legacy" | "compact") {
  const computer = new ComputerClient({
    baseUrl: "http://cptr.test",
    token: "benchmark-token",
    fetchImpl: async () => new Response("{}", { status: 200 }),
  });
  const server = createMcpServer(computer, { toolSurface });
  const client = new Client({ name: `contract-byte-benchmark-${toolSurface}`, version: "1" });
...
console.log(JSON.stringify({
  legacy: await measure("legacy"),
  compact: await measure("compact"),
}, null, 2));
